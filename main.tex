...
\usepackage{cite}           
\usepackage{caption}        
\captionsetup{labelsep=quad} % 图题冒号改为空格
...
% ===================== 第1章 引言 =====================
\section{引言}
微型飞行器 (MAVs)，特别是属于多旋翼类别的系统，如四旋翼和六旋翼飞行器，已成为一种广泛采用的空中机器人 。如今，此类系统被广泛用于自主巡检 \cite{A1}、监视 \cite{A2} 和其他遥感应用，以及涉及物理交互 \cite{A3}、配送 \cite{A4} 等任务 。它们的成功归因于多种因素，包括结构简单、低成本、可靠性高以及敏捷的动力学特性。其中一个关键组件与搭载在此类系统上的控制器的精度和鲁棒性有关，控制器与状态估计过程一并构成了促进自主导航所需的两个最基础的算法。
...
\begin{figure}[H] % [H] 表示强制图片在这里，不乱跑
    ...
    \caption{基于模型预测控制实现位置控制的导航无人机} % 修改图注
...
\begin{figure}[htbp]
    ...
    \caption{不同预测时域下线性与非线性 MPC 的阶跃响应对比}
    \label{fig:6}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{不同预测时域下线性与非线性 MPC 的计算时间对比}
    \label{fig:7}
\end{figure}
...
本文对模型预测控制 (MPC) 在微型飞行器 (MAVs) 上的应用和设计考量进行了综述。该领域的文献包括针对机器人飞行动力学的线性和非线性控制器、物理交互和负载运输的方法、容错控制方案以及结合现代强化学习技术的方法。随着 MAV 在重要应用领域的整合变得越来越广泛，我们预计对新型 MPC 方法的研究——特别是考虑到机器人导航过程中的不确定性和风险——将变得更加重要，并且对于实现鲁棒的自主飞行可能是必不可少的。
...
{\songti\zihao{-4}
    \begin{itemize}
        \item \textbf{徐振榕}：负责摘要、引言及建模部分撰写（第1-2章）。心得：深刻理解了MPC作为连接控制与规划的桥梁作用，掌握了在建模保真度与计算算力之间寻求平衡的方法，并认识到鲁棒性与智能化（如结合深度强化学习）是未来飞控发展的关键方向。
        \item \textbf{肖大亮}：负责线性MPC理论推导撰写（第3.1节）。心得：掌握了利用泰勒展开进行模型线性化的方法，理解了线性MPC在计算效率上的优势。
        \item \textbf{杨宇航}：负责非线性MPC及对比实验分析撰写（第3.2-3.3节）。心得：深入学习了非线性MPC在处理大机动飞行时的优势，并通过对比实验验证了其性能。
        \item \textbf{朱仕霖}：负责容错控制与深度强化学习部分撰写（第3.4-3.5节）。心得：了解了MPC在容错控制中的潜力，以及深度强化学习如何辅助MPC提升策略性能。
        \item \textbf{于椋一}：负责特殊应用场景及结论撰写（第3.6-4章）。心得：调研了MPC在负载运输和物理交互等复杂场景下的应用，对MAV控制的前沿有了更全面的认识。
    \end{itemize}
}
